...
export default function SavedInvoicePage() {
  ...
  const loadInvoices = useCallback(async () => {
    ...
    try {
      ...
      if (!currentUser?.id) {
        toast({
          title: "Error",
          description: "Anda harus login untuk melihat data invoice",
          variant: "destructive"
        })
        return
      }
      ...
      if (response.success) {
        ...
        toast({
          title: "Error",
          description: response.error || "Gagal mengambil data invoice",
          variant: "destructive"
        })
      }
    } catch (error) {
  ...
  }, [currentPage, searchTerm, dateFrom, dateTo, toast])
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 space-y-8">
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600">{100 - totalRecords}</p>
        ...
        <div className="flex justify-between items-center">
          ...
          {isAtLimit && (
            <Badge variant="destructive" className="text-sm">
              Batas maksimal 100 invoice tercapai
            </Badge>
          )}
        </div>
        ...
        <Card className="border-2 border-gray-200">
          <CardHeader>
            ...
            <CardDescription>
              {invoiceList.length} dari {totalRecords} invoice
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <Link href="/invoice">
                  <Button className="mt-4">Buat Invoice Pertama</Button>
                </Link>
              </div>
            ) : (
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <p className="text-gray-600 mb-1">
                            <strong>Dari:</strong> {invoice.applicantName}
                          </p>
                          <p className="text-gray-600 mb-1">
                            <strong>Kepada:</strong> {invoice.recipientName}
                          </p>
                          <p className="text-gray-600 mb-1">
                            <strong>Items:</strong> {invoice.items?.length || 0} item(s)
                          </p>
                          <p className="text-gray-600 mb-1">
                            <strong>Tanggal:</strong> {formatDate(invoice.createdDate)}
                          </p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <Card className="w-full max-w-md">
              <CardHeader>
                ...
                <CardDescription>
                  Apakah Anda yakin ingin menghapus invoice "{showDeleteDialog.name}"?
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex justify-end gap-2">
                  ...
                  <Button 
                    variant="outline" 
                    onClick={() => handleDeleteInvoice(showDeleteDialog.id, false)}
                  >
                    Soft Delete
                  </Button>
                  <Button 
                    variant="destructive" 
                    onClick={() => handleDeleteInvoice(showDeleteDialog.id, true)}
                  >
                    Hard Delete
                  </Button>
                </div>
